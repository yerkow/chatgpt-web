			...
			<div class="space-y-4">
				...
				<NTabs v-model:value="activeTab" type="line">
					<NTabPane name="login" :tab="$t('common.login')">
						...
						<NInput v-model:value="password" type="password" :placeholder="$t('common.password')" class="mb-2"
										@keypress="handlePress"/>
						<NInput v-if="need2FA" v-model:value="token" type="text" :placeholder="$t('common.twoFA')" class="mb-2"
										@keypress="handlePress"/>
					...
					<NTabPane v-if="authStore.session && authStore.session.allowRegister" name="register"
						...
						<NButton block type="primary" :disabled="disabled || password !== confirmPassword" :loading="loading"
					...
					<NTabPane name="resetPassword" :tab="$t('common.resetPassword')">
						<NInput v-model:value="username" :disabled="sign !== undefined" type="text"
										:placeholder="$t('common.email')" class="mb-2"/>
						<NInput v-if="!!sign" v-model:value="password" type="password" :placeholder="$t('common.password')"
										class="mb-2" @input="handlePasswordInput"/>
						<NInput
							v-if="showConfirmPassword"
							v-model:value="confirmPassword"
							type="password"
							...
							:status="confirmPasswordStatus"
						/>
						<NButton v-if="!sign" block type="primary" :disabled="username.length <= 0" :loading="loading"
										 @click="handleSendResetMail">
							{{ $t('common.resetPasswordMail') }}
						</NButton>
						<NButton v-else block type="primary" :disabled="disabled || password !== confirmPassword" :loading="loading"
