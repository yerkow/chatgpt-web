...
import {
    ...
    checkUserVerify,
    checkUserVerifyAdmin,
    getUserResetPasswordUrl,
    getUserVerifyUrl,
    getUserVerifyUrlAdmin,
    md5
} from './utils/security'
    ...
    try {
        ...
        if (isNotEmptyString(config.siteConfig.registerMails)) {
            ...
            if (!allowSuffix) {
                res.send({
                    status: 'Fail',
                    message: '该邮箱后缀不支持 | The email service provider is not allowed',
                    data: null
                })
                return
            }
        ...
        if (isRoot) {
            res.send({status: 'Success', message: 'Регистрация прошла успешно! | Register success', data: null})
        } else {
            await sendVerifyMail(username, await getUserVerifyUrl(username))
            res.send({
                status: 'Success',
                message: '注册成功, 去邮箱中验证吧 | Registration is successful, you need to go to email verification',
                data: null
            })
        }
    } catch (error) {
    ...
    try {
        ...
        if (user.secretKey) {
            if (token) {
                ...
                res.send({
                    status: 'Success',
                    message: '需要两步验证 | Two-step verification required',
                    data: {need2FA: true}
                })
                return
            }
...
router.post('/user-edit', rootAuth, async (req, res) => {
    try {
        const {userId, email, password, roles, remark, useAmount, limit_switch} = req.body as {
            userId?: string;
            email: string;
            password: string;
            roles: UserRole[];
...
router.post('/user-password', auth, async (req, res) => {
    try {
        let {oldPassword, newPassword, confirmPassword} = req.body as {
            oldPassword: string;
            newPassword: string;
...
router.post('/audit-test', rootAuth, async (req, res) => {
    try {
        ...
        res.send({
            status: 'Success',
            message: result ? '含敏感词 | Contains sensitive words' : '不含敏感词 | Does not contain sensitive words.',
            data: null
        })
    } catch (error) {
        res.send({status: 'Fail', message: error.message, data: null})
